...
class Dobot(Robot, object):
    ...
    def __init__(self, node_name="painting"):
        return
        # self.debug_bool = debug
    ...
    def good_night_robot(self):
        print("Disconnect")
        dobot_api.DobotApi.__del__()

    def go_to_cartesian_pose(self, positions, orientations, precise=False):
        # positions = [565, 157, 240]
        # orientations = [0.065, -0.955, -0.286, 0.035]
        ...
        for i in range(len(positions)):
            ...
            roll, pitch, yaw = euler[0]/math.pi*180.0, euler[1]/math.pi*180.0, euler[2]/math.pi*180.0 # in degree

            nxt_point = [x,y,z,roll,pitch,yaw]
            print(nxt_point)
            self.RunPoint(move, nxt_point)
            # self.WaitArrive(nxt_point)
    ...
    def ConnectRobot(self):
        try:
            ...
            feedPort = 30004
            print("正在建立连接...")
            dashboard = dobot_api.DobotApiDashboard(ip, dashboardPort)
            move = dobot_api.DobotApiMove(ip, movePort)
            feed = dobot_api.DobotApi(ip, feedPort)
            print(">.<连接成功>!<")
    ...
    def RunPoint(self, move: dobot_api.DobotApiMove, point_list: list):
        #print(point_list)
    ...
    def GetFeed(self, feed: dobot_api.DobotApi):
        global current_actual
        global algorithm_queue
        global enableStatus_robot
        global robotErrorState
        global globalLockValue
        hasRead = 0
        while True:
            data = bytes()
            while hasRead < 1440:
                ...
                if len(temp) > 0:
                    ...
                    data += temp
            hasRead = 0
            feedInfo = np.frombuffer(data, dtype=dobot_api.MyType)
    ...
    def ClearRobotError(self, dashboard: dobot_api.DobotApiDashboard):
        global robotErrorState
        dataController, dataServo = dobot_api.alarmAlarmJsonFile()    # 读取控制器和伺服告警码
        while True:
            self.globalLockValue.acquire()
